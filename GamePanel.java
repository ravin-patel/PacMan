...
public class GamePanel extends JPanel{
	...
	public GamePanel() {
		...
		this.setBackground(Color.decode( "#088A08" ));
	...
	class TimeListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			for (Creature c : creatures) { 
				...
				if (predatorCreature.getX() < 0) predatorCreature.reverseSignal();
				if (predatorCreature.getX() > frame.FRAME_WIDTH-60) predatorCreature.reverseSignal();
				if (predatorCreature.getY() < 0) predatorCreature.reverseSignal();
				if (predatorCreature.getY() > frame.FRAME_HEIGHT-60) predatorCreature.reverseSignal();
	...
	public void paintComponent(Graphics g) {
		...
		if (firstStart) {
			...
			g2.drawString("Control PacMan with the mouse and eat all of the ghosts", 10,50);
			g2.drawString("Fast Prey(Green) = 5 Points", 10, 70);
